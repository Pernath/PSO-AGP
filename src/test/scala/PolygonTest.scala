...
class FirstSpec extends FlatSpec {
  ...
  "dentro()" should "decide if point is inside polygon" in {
    ...
    var fuera = new Punto((w.puntos(3).x+w.puntos(1).x)/2,(w.puntos(3).y+w.puntos(1).y)/2)
    println(fuera)

    assert(!w.dentro(fuera))
  ...
  "segmento_dentro()" should "decide if line segment lies inside the polygon" in {
    ...
    assert(!w.segmento_dentro(w.puntos(3),w.puntos(1)))

    
    assert(w.segmento_dentro(w.puntos(11),w.puntos(0)))
    println("gola: "+((w.puntos(11).x+w.puntos(10).x)/2,(w.puntos(11).y+w.puntos(10).y)/2))
    assert(w.dentro(new Punto((w.puntos(11).x+w.puntos(10).x)/2,(w.puntos(11).y+w.puntos(10).y)/2)))
